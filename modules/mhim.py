...
class MHIM(nn.Module):
    def __init__(self, mlp_dim=512,channel_dim=1024,mask_ratio=0,n_classes=2,temp_t=1.,temp_s=1.,dropout=0.25,act='relu',select_mask=True,select_inv=False,msa_fusion='vote',mask_ratio_h=0.,mrh_sche=None,mask_ratio_hr=0.,mask_ratio_l=0.,da_act='gelu',baseline='selfattn',head=8,attn_layer=0):
        ...
        self.patch_to_emb = [nn.Linear(channel_dim, 512)]
        ...
        elif baseline == 'mymamba':
            self.online_encoder = MAMBA_block_MY(mlp_dim=mlp_dim)
    ...
    def forward(self, x, attn=None, teacher_cls_feat=None, i=None):
        x = self.patch_to_emb(x) # [1, N, 512]
        x = self.dp(x)
        ps = x.size(1)

        if self.select_mask and attn is not None:
            if attn is not None and isinstance(attn,(list,tuple)):
                if self.attn_layer == -1:
                    attn = attn[1]
                else:
                ...
                attn = attn
            len_keep_h, mask_ids_h = self.select_mask_fn(ps, attn, largest=True, mask_ratio=self.mask_ratio_h)
            len_keep_l, mask_ids_l = self.select_mask_fn(ps, attn, largest=False, mask_ratio=self.mask_ratio_l)
            
            feat_high = self.online_encoder(x, len_keep=len_keep_h, mask_ids=mask_ids_h, mask_enable=True)
            feat_low = self.online_encoder(x, len_keep=len_keep_l, mask_ids=mask_ids_l, mask_enable=True)
            
            cont_loss = torch.mean(F.cosine_similarity(feat_high, feat_low)) 
        else:
            feat_high = self.online_encoder(x)
            cont_loss = 0.

        student_logit = self.predictor(feat_high)
        cls_loss = self.forward_loss(student_cls_feat=feat_high, teacher_cls_feat=teacher_cls_feat)

        total_loss = cls_loss - 0.01 * cont_loss 

        return student_logit, total_loss, ps, len_keep_h