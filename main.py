...
import pandas as pd
def check_fold_distribution(train_labels, val_labels, test_labels):
    stats = []
    num_folds = len(train_labels)
    
    for i in range(num_folds):
        train_dist = Counter(train_labels[i])
        val_dist = Counter(val_labels[i])
        test_dist = Counter(test_labels[i])
        all_classes = set(train_dist.keys()).union(set(val_dist.keys()), set(test_dist.keys()))
        for cls in sorted(all_classes):
            stats.append({
                ...
                "Total_in_Fold": train_dist.get(cls, 0) + val_dist.get(cls, 0) + test_dist.get(cls, 0)
            })
...
def main(args):
    ...
    # --->get dataset
    if args.datasets.lower() == 'dataset1':
        label_path=os.path.join(args.dataset_root,'label_dataset1.csv')
    ...
    check_fold_distribution(train_l, val_l, test_l)
...
def one_fold(args,k,ckc_metric,train_p, train_l, test_p, test_l,val_p,val_l):
    ...
    if args.datasets.lower() == 'dataset1':
        ...
        train_labels_fold = train_l[k]
        class_weights = compute_class_weights(train_labels_fold, args.n_classes)
    ...
    if args.model == 'mhim':
        ...
        model = mhim.MHIM(select_mask=False,channel_dim=args.channel_dim,n_classes=args.n_classes,act=args.act,head=args.n_heads,da_act=args.da_act,baseline=args.baseline).to(device)
    ...
    if args.early_stopping:
        early_stopping = EarlyStopping(patience=30 if args.datasets=='dataset1' else 20, stop_epoch=args.max_epoch if args.datasets=='dataset1' else 70,save_best_model_stage=np.ceil(args.save_best_model_stage * args.num_epoch))
    else:
        early_stopping = None
    ...
    for epoch in range(epoch_start, args.num_epoch):
        ...
        res_val = val_loop(args, model, val_loader, device, criterion, early_stopping, epoch, model_tea)
        stop, accuracy, auc_value, precision, recall, fscore, specificity,test_loss, threshold_optimal, per_class_info,raw_labels, raw_hat, raw_prob = res_val
        class_names = ["Class_0", "Class_1", "Class_2", "Class_3"] 
        per_class_log = ""
        for i in range(len(class_names)):
            per_class_log += f"\n  - [{class_names[i]}] auc: {per_class_info['auc'][i]:.4f} | accuracy: {per_class_info['accuracy'][i]:.4f} | Fscore: {per_class_info['fscore'][i]:.4f} | precision: {per_class_info['precision'][i]:.4f} | recall: {per_class_info['recall'][i]:.4f} | specificity: {per_class_info['specificity'][i]:.4f}"

        if not args.no_log:
            log_msg = (f"Epoch [{epoch+1}/{args.num_epoch}] "
                    f"Train Loss: {train_loss:.1E}, Val Loss: {test_loss:.1E}, "
                    f"accuracy: {accuracy:.4f}, auc: {auc_value:.4f}, precision: {precision:.4f}, recall: {recall:.4f},fscore: {fscore:.4f},specificity: {specificity:.4f},Time: {train_time_meter.val:.2f}")
            
            print(log_msg + per_class_log) 

            with open(os.path.join(args.model_path, f'fold_{k}_detailed_log.txt'), 'a') as f:
                f.write(log_msg + per_class_log + '\n\n')
        ...
        if args.wandb:
            ...
            for i in range(4):
                rowd[f"per_class/C{i}_auc"] = per_class_info['auc'][i]
                rowd[f"per_class/C{i}_f1"] = per_class_info['fscore'][i]

            rowd_wandb = OrderedDict([(str(k)+'-fold/'+_k, _v) for _k, _v in rowd.items()])
            wandb.log(rowd_wandb)

        if model_tea is not None:
            _,accuracy_tea, auc_value_tea, precision_tea, recall_tea, fscore_tea, specificity_tea, test_loss_tea,_,_,_,_,_ = val_loop(args,model_tea,val_loader,device,criterion,None,epoch,model_tea)
            if args.wandb:
                rowd = OrderedDict([
        ...
        if not args.no_log:
            log_msg = '\r Epoch [%d/%d] train loss: %.1E, test loss: %.1E, accuracy: %.3f, auc_value: %.3f, precision: %.3f, recall: %.3f, fscore: %.3f, time: %.3f(%.3f)' % (
                epoch+1, args.num_epoch, train_loss, test_loss, accuracy, auc_value, precision, recall, fscore, train_time_meter.val, train_time_meter.avg
            )
            
            print(log_msg)

            with open('mamba_stage1.txt', 'a') as f:
                f.write(log_msg + '\n')
        if args.wandb:
            rowd = OrderedDict([
                ("val_acc",accuracy),
        ...
        if auc_value > opt_auc and epoch >= args.save_best_model_stage*args.num_epoch:
            ...
            if not args.no_log:
                ...
                torch.save(epoch_results, os.path.join(args.model_path, 'fold_{fold}_model_best_auc_results.pt'.format(fold=k)))
        ckp = {
            'model': model.state_dict(),
            'teacher': model_tea.state_dict() if model_tea is not None else None,
        }

        fold_dir = os.path.join(args.model_path, f'fold_{k}')
        os.makedirs(fold_dir, exist_ok=True)
        checkpoint_name = f'fold_{k}/fold_{k}_epoch_{epoch+1}.pt'
        torch.save(ckp, os.path.join(args.model_path, checkpoint_name))

        epoch_res_path = os.path.join(fold_dir, f'fold_{k}_epoch_{epoch+1}_results.pt')
        torch.save(epoch_results, epoch_res_path)
    ...
    accuracy, auc_value, precision, recall, fscore,specificity,test_loss_log,per_class_test,_,_,_ = test(args,model,test_loader,device,criterion,model_tea,opt_thr)
    
    if not args.no_log:
        print('\n' + '#' * 20 + ' FINAL TEST RESULTS ' + '#' * 20)
        print(f'Optimal Avg AUC: {opt_auc:.3f}')
        print(f'Test Avg AUC: {auc_value:.3f} | Acc: {accuracy:.3f}')

    for i in range(4):
        print(f'  Class {i}: AUC={per_class_test["auc"][i]:.3f}, F1={per_class_test["fscore"][i]:.3f}')
...
def val_loop(args,model,loader,device,criterion,early_stopping,epoch,model_tea=None):
    ...
    avg_scores, per_class_info = five_scores(bag_labels, bag_hat, bag_prob)
    accuracy, auc_value, precision, recall, fscore,specificity,threshold_optimal = avg_scores
    # early stop
    ...
    return stop, accuracy, auc_value, precision, recall, fscore, specificity,loss_cls_meter.avg, threshold_optimal, per_class_info, bag_labels, bag_hat, bag_prob
...
def test(args,model,loader,device,criterion,model_tea=None,opt_thr=None):
    ...
    # accuracy, auc_value, precision, recall, fscore,threshold_optimal = five_scores(bag_labels, bag_hat, bag_prob) 
    avg_scores, per_class_info = five_scores(bag_labels, bag_hat, bag_prob)
    acc, auc, pre, rec, fs,specificity,threshold_optimal = avg_scores
 
    # test_loss_log = test_loss_log/len(loader)

    # return accuracy, auc_value, precision, recall, fscore,test_loss_log
    return acc, auc, pre, rec, fs, specificity,test_loss_log,per_class_info, bag_labels, bag_hat, bag_prob

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='MIL Training Script')

    # Dataset 
    parser.add_argument('--datasets', default='dataset1', type=str, help='[camelyon16, tcga, dataset1]')
    ...
    parser.add_argument('--channel_dim', default=1024, type=int)
    ...
    parser.add_argument('--project', default='NIMM', type=str, help='Project name of exp')
