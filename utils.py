...
def five_scores(bag_labels, bag_hat, bag_prob):
    # Ensure numpy to tensor, long dtype, and 1D shape
    bag_labels = torch.tensor(bag_labels, dtype=torch.long).view(-1)
    bag_hat = torch.tensor(bag_hat, dtype=torch.long).view(-1)

    # AUC 依赖于 prob 是 numpy 或 tensor 的概率分布，shape = [N, num_classes]
    auc_value = roc_auc_score(bag_labels.numpy(), bag_prob, average='macro', multi_class='ovo')

    metrics = torchmetrics.MetricCollection([
        torchmetrics.Accuracy(task='multiclass', num_classes=4, average='micro'),
        torchmetrics.CohenKappa(task='multiclass', num_classes=4),
    ...
    return (results['MulticlassAccuracy'].item(),
            auc_value,
